...
import type { PortfolioItem } from "@/sanity/queries";
...
function ProjectCard({ title, tags, img, alt, height }: { title: string; tags: string[]; img: string; alt: string; height: string }) {
  return (
...
const PORTFOLIO_HEIGHTS = ["744px", "699px", "699px", "744px"] as const;

function portfolioCardSrc(item: PortfolioItem, width: number, height: number) {
  return urlFor(item.image).width(width).height(height).fit("crop").auto("format").url();
}

function PortfolioCard({ item, slot }: { item: PortfolioItem; slot: 0 | 1 | 2 | 3 }) {
  const height = PORTFOLIO_HEIGHTS[slot];
  const numericHeight = parseInt(height, 10);
  return (
    <ProjectCard
      title={item.title}
      tags={item.tags ?? []}
      img={portfolioCardSrc(item, 800, numericHeight)}
      ...
      <div className="flex flex-col gap-[61px] w-full">
        ...
        <div className="flex gap-6 items-end w-full">
          <div className="flex flex-1 flex-col justify-between min-w-0 gap-6">
            {left.map((item, i) => (
              <PortfolioCard key={item._id} item={item} slot={i as 0 | 1} />
            ))}
          ...
          <div className="flex flex-1 flex-col gap-[117px] pt-[420px] min-w-0">
            {right.map((item, i) => (
              <PortfolioCard key={item._id} item={item} slot={(i + 2) as 2 | 3} />
            ))}
          </div>
        </div>
      </div>
...
export default function HomeClient({ portfolioItems }: { portfolioItems: PortfolioItem[] }) {
  return (
